<?php
include $_SERVER['DOCUMENT_ROOT']."/MK_home/mk_session_call.php";
...
if(isset($_GET["mode"])&&$_GET["mode"]=="insert"){
    ...
    echo "<script>location.href='./mk_list.php';</script>";
    ...
    mysqli_close($conn);
    echo "<script>location.href='./mk_list.php?page=1';</script>";
  ...
  echo "<script>location.href='./mk_view.php?num=$num&hit=$hit';</script>";
  ...
  $sql="INSERT INTO `qna` VALUES (null,$group_num,$depth,$ord,
    '$q_userid','$username','$q_subject','$q_content','$regist_day',$hit,'$is_html');";
  $result = mysqli_query($conn,$sql);
  if (!$result) {
    die('Error: ' . mysqli_error($conn));
  }
  ...
  echo "<script>location.href='./mk_view.php?num=$max_num&hit=$hit';</script>";
